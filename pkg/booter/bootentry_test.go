...
func TestGetBooterForStBooter(t *testing.T) {
	validConfig := BootEntry{
		Name:   "Boot0000",
		Config: []byte(`{"type": "stboot"}`),
	}
	booter := GetBooterFor(validConfig)
	require.NotNil(t, booter)
	require.Equal(t, booter.TypeName(), "stboot")
	require.NotNil(t, booter.(*StBooter))
}
...
func TestGetBootEntries(t *testing.T) {
	var (
		...
		bootConfig0002 = []byte(`{"type": "stboot"}`)
	)
