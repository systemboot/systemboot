...
func UpdateLinuxRandomness() error {
	...
	if !good {
		Die("No valid TRNG config found. Details: "+err.Error(), false, rebootTimeout)
	}

	hwRng, err := os.Open(hwRandomDevice)
	if err != nil {
		Die("Can't open hw random device. Details: "+err.Error(), false, rebootTimeout)
	}

	rng, err := os.Open(randomDevice)
	if err != nil {
		Die("Can't open /dev/random device. Details: "+err.Error(), false, rebootTimeout)
	}
